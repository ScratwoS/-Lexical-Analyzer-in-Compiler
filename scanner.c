...
TokenType getToken() {
    ...
    if (isalpha(Ch)) {
        ...
        switch (Ch) {
            case '+': getCh(); return (Token = PLUS);
            case '-': getCh(); return (Token = MINUS);
            case '*': getCh(); return (Token = TIMES);
            case '/': getCh(); return (Token = SLASH);
            case '=': getCh(); return (Token = EQU);
            case '<':
                getCh();
                if (Ch == '=') { getCh(); return (Token = LEQ); }
                else if (Ch == '>') { getCh(); return (Token = NEQ); }
                else return (Token = LSS);
            case '>':
                getCh();
                if (Ch == '=') { getCh(); return (Token = GEQ); }
                else return (Token = GTR);
            case '(': getCh(); return (Token = LPARENT);
            case ')': getCh(); return (Token = RPARENT);
            case '[': getCh(); return (Token = LBRACK);
            case ']': getCh(); return (Token = RBRACK);
            case '.': getCh(); return (Token = PERIOD);
            case ',': getCh(); return (Token = COMMA);
            case ';': getCh(); return (Token = SEMICOLON);
            case ':':
                getCh();
                if (Ch == '=') {
                    getCh();
            ...
            case '%': getCh(); return (Token = PERCENT);
            case EOF: return (Token = NONE);
            default:
